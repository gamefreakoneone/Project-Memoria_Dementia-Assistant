"""
Enhanced Time Agent for Dementia Assistance

This agent helps dementia patients recall their activities by querying
MongoDB event data and generating digestible timeline summaries.

Improvements over original:
- Pydantic models for structured, type-safe responses
- Robust error handling with graceful degradation
- Semantic search for activity verification (handles synonyms)
- Better time parsing with more natural language support
- Connection cleanup for resource management

Supports queries like:
- "What was I doing yesterday?"
- "What was I doing in my bedroom?"
...
class TimelineResult(BaseModel):
    """Result containing a digestible timeline of activities"""
...
class TranscriptResult(BaseModel):
    """Result containing audio transcripts"""
...
class ActivityCheckResult(BaseModel):
    """Result from checking for specific activities like medication"""
...
# ============================================================================
# Enhanced Time Agent Class
# ============================================================================


class TimeAgent:
    def __init__(self, output_model: str = "gpt-4o"):
        self.client = AsyncOpenAI(api_key=os.getenv("OPENAI_API_KEY"))
        self.mongo_uri = os.getenv("MONGODB_URI", "mongodb://localhost:27017")
        self._mongo_client = None
        self.output_model = output_model

        # Room mapping (could be passed in or fetched dynamically)
    ...
    async def _summarize_with_llm(
        ...
        for e in events:
            ...
            item = {
                "time": time_str,
                "room": e.room_name,
                "activity": e.video_description[:200] if e.video_description else "",
                "speech": e.audio_transcript[:200] if e.audio_transcript else "",
            }
...
- Keep your response to 2-3 sentences"""
    ...
    # ========================================================================
    # Tool Functions with Structured Responses
    # ========================================================================
    ...
    async def get_room_activity(
        self, room_name: str, time_range: str = "today"
    ) -> TimelineResult:
        """
        Get activities for a specific room.

        Args:
            room_name: Name of room (e.g., "bedroom", "living room") or room ID.
            time_range: "today", "yesterday", "recently", "last N hours", or YYYY-MM-DD.

        Returns:
            TimelineResult with activities in that room.
        """
        try:
            room_id = self._parse_room_name(room_name)

            if room_id is None:
                available_rooms = ", ".join(self.rooms.values())
                return TimelineResult(
                    success=False,
                    event_count=0,
                    time_range=time_range,
                    summary=f"I don't recognize '{room_name}'. "
                    f"The rooms I know about are: {available_rooms}.",
                )

            start_dt, end_dt, time_desc = self._build_time_filter(time_range)
            events = await self._get_events(start_dt, end_dt, room_number=room_id)
            room_display = self.rooms.get(room_id, f"Room {room_id}")

            if not events:
                return TimelineResult(
                    success=False,
                    event_count=0,
                    time_range=f"{time_desc} in {room_display}",
                    summary=f"I don't have any recorded activities in the {room_display} "
                    f"for {time_desc}.",
                )

            summary = await self._summarize_with_llm(
                events, f"What was I doing in the {room_display}?"
            )
